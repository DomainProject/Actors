...
"""
def query1(df):
    windowed_df = df.withWatermark("tpep_pickup_datetime", "1 hour").groupBy(
        window("tpep_pickup_datetime", "1 day"), "PULocationID"
    ).sum("passenger_count")
...
def query2(df):
    windowed_df = df \
        ...
        .select("*", window(col("tpep_pickup_datetime"), "1 day").alias("window"))
